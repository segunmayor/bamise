...
/**
 * Extends the standard request adapter to include $_FILES metadata in input()
...
final class PhpRequest implements CrudRequestInterface
{
    ...
    public function __construct()
    {
        ...
        if ($this->method === 'POST') {
            // Merge $_POST with $_FILES metadata so middleware can detect uploads
            $filesInfo = [];
            if (isset($_FILES['file']) && is_array($_FILES['file'])) {
                $filesInfo['_uploaded_file'] = [
            ...
            $this->input = array_merge($_POST, $filesInfo);
        } elseif (in_array($this->method, ['PUT', 'PATCH'], true)) {
